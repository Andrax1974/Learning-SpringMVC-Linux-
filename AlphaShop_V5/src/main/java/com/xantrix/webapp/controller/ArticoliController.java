...
{
	private static final Logger logger = LogManager.getLogger("Articoli");
			...
			@MatrixVariable(pathVar = "parametri") Map<String, List<String>> parametri, Model model)
	{
		...
		//Eseguiamo il riordino del recordset
		switch (orderBy)
		{
			case "codart":
				if (tipo.equals("asc"))
				{
					recordset = recordset.stream()
							.sorted(Comparator.comparing(Articoli::getCodArt))
							.collect(Collectors.toList()); 
				}
				else
				{
					recordset = recordset.stream()
							.sorted(Comparator.comparing(Articoli::getCodArt).reversed())
							.collect(Collectors.toList()); 
				}
				break;
			case "descrizione":
				...
				{
					recordset = recordset.stream()
							.sorted(Comparator.comparing(Articoli::getDescrizione))
							.collect(Collectors.toList()); 
				}
				else
				{
					recordset = recordset.stream()
							.sorted(Comparator.comparing(Articoli::getDescrizione).reversed())
							.collect(Collectors.toList()); 
				}
				break;
		}
			...
			Model model)
	{
		...
		recordset = recordset
				.stream()
				.sorted(Comparator.comparing(Articoli::getDataCreaz).reversed())
				.limit(NumArt)
				.collect(Collectors.toList());
			...
			@PathVariable("CodArt") String CodArt, Model model, HttpServletRequest request)
	{
		...
		{
			logger.info("Modifica Articolo " + articolo);
			articoliService.InsArticolo(articolo);
	...
	public String DelArticolo(@PathVariable("CodArt") String codArt, Model model)
	{
		try
		{
			logger.info("Eliminazione Articolo con Codice: " + codArt);
